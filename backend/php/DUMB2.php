...
try {
    ...
    $scan2_time = date('Y-m-d H:i:s'); // IST time
    ...
    if ($checkResult->num_rows > 0) {
        // Update scan2_time
        $updateStmt = $conn->prepare("UPDATE guest_details SET scan2_time = ? WHERE roll_number = ?");
        if (!$updateStmt) {
            throw new Exception("Update preparation failed: " . $conn->error);
        }

        $updateStmt->bind_param("ss", $scan2_time, $roll_number);
        $updateStmt->execute();

        if ($updateStmt->affected_rows === 0) {
            throw new Exception("Failed to update scan2_time. No rows affected.");
        }

        $response = [
            'status' => 'success',
            'message' => 'scan2 time updated successfully',
            'scan2_time' => $scan2_time
        ];
